...
def filter_product_by_tags(qs, tag_pks):
    return qs.filter(tags__in=tag_pks).distinct()
...
def filter_tags(qs, _, value):
    if value:
        _, tag_pks = resolve_global_ids_to_primary_keys(value, product_types.Tag)
        qs = filter_product_by_tags(qs, tag_pks)
    return qs
...
class ProductFilter(MetadataFilterBase):
    ...
    tags = GlobalIDMultipleChoiceFilter(method=filter_tags)
