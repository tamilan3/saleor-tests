...
class Product(ChannelContextTypeWithMetadata[models.Product]):
    ...
    tag = NonNullList("saleor.graphql.product.types.Tag")
    previous = graphene.GlobalID()
    next = graphene.GlobalID()
    ...
    @staticmethod
    def resolve_tag(root: ChannelContext[models.Product], _info) -> Optional[int]:
        product = root.node
        return product.tags.all()

    @staticmethod
    def resolve_previous(root: ChannelContext[models.Product], _info) -> Optional[int]:
        product = root.node
    ...
    @staticmethod
    def resolve_next(root: ChannelContext[models.Product], _info) -> Optional[int]:
        product = root.node
        if not product:
            return None

        next_product = models.Product.objects.filter(
            id__gt=product.id
        ).order_by('id').values_list('id', flat=True).first()
